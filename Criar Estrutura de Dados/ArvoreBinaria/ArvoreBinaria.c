...
struct NO* remove_atual(struct NO* atual) {
    struct NO *no1, *no2;
    if(atual->esq == NULL){ //Sem filho da esquerda. Apontar para o filho da direita (trata nó folha e nó com 1 filho)

        no2 = atual->dir;
        free(atual);
        return no2;
    }
    no1 = atual; //Procura filho mais a direita na sub-árvore da esquerda.
    ...
    // no1 eh o pai de no2
    if(no1 != atual){ //Copia o filho mais a direita na sub-árvore da esquerda para o lugar do nó removido.
        no1->dir = no2->esq;
        no2->esq = atual->esq;
    }
    no2->dir = atual->dir;
